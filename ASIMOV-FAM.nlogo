...
to setup-Environment
  ...
  ifelse member? "Sequence Learning" experiment_mode or member? "Population" experiment_mode[; changes prey locations, environment wrapping, and walls depending on experiment mode
    ...
    ifelse experiment_mode = "3 Source Spatial Mapping"  [
      ...
      create-prey 1 [set prey-type "p7" setxy -41 -50 set fixed 1]
...
to calculate-traces
  ...
  let bdist sign2 slitherometer
  (foreach indices [[n] ->
    ifelse n < num_inputs - 2 [
      let trace item n traces
      let others remove-item n inputs
      let other-sum sum others + 10 * (reward_pos + abs reward_neg)
      ...
      ifelse item n inputs > 1e-7 [set trace 0.7 + 0.3 / (1 + exp(-3 * (item n inputs) + 7))] [set trace (1 - bdist)*(trace - 0.03 * trace - 1.94 * speed) + bdist * (trace - 0.03 * trace * other-sum)]
  ...
  if bdist = 1 and max inputs <= 1e-7[ ;during path integration and absence of inputs, ensure that at most 1 sensory trace is active
    let max_traces max traces
    let max_frequency occurrences max_traces traces
    foreach indices [[n] ->
    if n < num_inputs - 2 [
        let trace item n traces
        ;if trace < max_traces or max_frequency > 1 [set traces replace-item n traces 0]
      ]
    ]
  ]
end
...
The top left hand side of the interface provides the initialization of the simulation, with _different modes available under **Experiment_Mode**, which include demos of temporal and spatial sequence learning, as well as spatial mapping_. The Setup button initializes the chosen experiment mode simulation, and the Go button runs it. The Reset to Default Settings button restores all variables to their default values. 
