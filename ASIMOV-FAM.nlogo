...
globals [data Addiction_Cycle_Phase switch_time stop_time prey-types-list prey-color-list odor-color-list odor-types-list num-odor-types prey-odor-matrix diffc evapc
         cluster-x-coordinates cluster-y-coordinates plot-timer wallcount dcount Presenting Present-y-coord pstep pticks Static-Environment Spatial-Mapping-Enabled]
...
Cslugs-own [ Somatic_Map App_State App_State_Switch ExpReward_pos ExpReward_neg iSum Incentive Nutrition Satiation speed turn-angle
             ...
             num_senses sense_names senses imag_senses senses_left imag_senses_left senses_right imag_senses_right sense_colors
             ...
             i j FAMatrix_dim init_list FAMatrix_diff-A FAMatrix_cross-A FAMatrix_strengths FAMatrix_timelags FAMatrix_rewards FAMatrix_vdir FAMatrix_vdist FAMatrix_vseq reward_pos reward_neg
...
to setup
  ...
  setup-ASIMOV-Agent
  setup-Environment
  ask Cslugs [update-arms pen-down] ;Track Cslug's path
end
...
to setup-Environment
  ...
  ifelse member? "Sequence Learning" experiment_mode or member? "Population" experiment_mode[; changes prey locations, environment wrapping, and walls depending on experiment mode
    ...
    ifelse experiment_mode = "3 Source Spatial Mapping"  [
      ...
      create-prey 1 [set prey-type "flab" setxy -19 -23 set fixed 1]
      create-prey 1 [set prey-type "drug" setxy 55 -36 set fixed 1]
    ][
      create-prey 1 [set prey-type "hermi" setxy 11 40 set fixed 1] ;0 34
      create-prey 1 [set prey-type "flab" setxy 2 -45 set fixed 1] ;1 -20
      create-prey 1 [set prey-type "drug" setxy 45 -7 set fixed 1] ;-34 -68
      create-prey 1 [set prey-type "p5" setxy -36 39 set fixed 1]
      ;create-prey 1 [set prey-type "p6" setxy 45 -46 set fixed 1]
      create-prey 1 [set prey-type "p7" setxy -46 -49 set fixed 1]
...
to Go
  ...
  ask Cslugs [
    ...
    rt turn-angle + 1 * (item (length(senses_left) - 2) senses_left) ; left turn bias for walls
    ...
    ifelse immobilize = true[
      ...
      set speed 0.5
      fd speed - (item (length(senses) - 2) senses) / 4 ; stops at walls
    ]
  ]
...
to calculate-Somatic-Map
  let Somatic_Map_senses ( lput sns-pain but-first sns_odors )
  let Somatic_Map_senses_left ( lput sns-pain-left but-first sns_odors_left )
  let Somatic_Map_senses_right ( lput sns-pain-right but-first sns_odors_right )

  set Somatic_Map_Factors map [x -> 2 * x - sum (Somatic_Map_senses)] Somatic_Map_senses
  let last_index (length Somatic_Map_Factors - 1)
  set Somatic_Map_Factors replace-item last_index Somatic_Map_Factors pain

  let indices range (length Somatic_Map_senses)
  ;
  ...
  (foreach indices Somatic_Map_senses_left Somatic_Map_senses_right Somatic_Map_Factors [[n sns_left sns_right C] ->
    set Somatic_Map_Sigmoids replace-item n Somatic_Map_Sigmoids ((sns_left - sns_right) / (1 + exp (-50 * C))) ])
  set Somatic_Map -1 * (sum Somatic_Map_Sigmoids)
end
  ...
  while [i < num_inputs] [
    while [j < num_inputs] [
      if i != j [
        ...
        let th 2
        if i < num_inputs - 2 and j < num_inputs - 2[
          let order matrix:get FAMatrix_timelags i j
          let expectedReward matrix:get FAMatrix_rewards i j
          set iSum iSum + expectedReward * (3 * (inputiL + inputjL) * (th - order * inputiL) * (th + order * inputjL)) / (num_inputs ^ 2)
          set iSum iSum + expectedReward * (3 * (inputiL + inputjR) * (th - order * inputiL) * (th + order * inputjR)) / (num_inputs ^ 2)
          set iSum iSum + expectedReward * (3 * (inputiR + inputjR) * (th - order * inputiR) * (th + order * inputjR)) / (num_inputs ^ 2)
          set iSum iSum + expectedReward * (3 * (inputiR + inputjL) * (th - order * inputiR) * (th + order * inputjL)) / (num_inputs ^ 2)

        ]
        if i < num_inputs - 2 and j >= num_inputs - 2[
          let expectedReward matrix:get FAMatrix_rewards i j
          set iSum iSum + 5 * expectedReward * (inputi) / (num_inputs ^ 2)
        ]
        if i >= num_inputs - 2 and j < num_inputs - 2[
          let expectedReward matrix:get FAMatrix_rewards i j
          set iSum iSum + 5 * expectedReward * (inputj) / (num_inputs ^ 2)
        ]
      ]
      set j j + 1
    ]
...
;
;=========================================================================================;
;sets eligibility traces for inputs
to calculate-traces
  ...
  (foreach indices [[n] ->
    ifelse n < num_inputs - 2 [
      ...
      ;ifelse item n inputs > 1e-7 [set trace 0.7 + 0.3 / (1 + exp(-3 * (item n inputs) + 7))] [set trace trace - (0.03 * trace * (1 - sign2 slitherometer)) - (0.03 * trace * sum others)]
      ;ifelse item n inputs > 1e-7 [set trace 0.7 + 0.3 / (1 + exp(-3 * (item n inputs) + 7))] [set trace (1.94 * (0.5 - speed)) * trace - (0.03 * trace * sum others)]
      ...
      ;ifelse item n inputs > 1e-7 [set trace 0.7 + 0.3 / (1 + exp(-3 * (item n inputs) + 7))] [set trace trace - (0.06 * (0.5 - speed) * trace) - (0.03 * trace * sum others)]
      ;ifelse item n inputs > 1e-7 [set trace 0.7 + 0.3 / (1 + exp(-3 * (item n inputs) + 7))] [ifelse speed = 0 [set trace 0.98 * trace][set trace trace - (0.03 * trace * sum others)]]
      ;set trace 1 / (1 + exp(-3 * (item n inputs - sum others) + 7))
    ...
    while [j < num_inputs] [
      if i != j[
        if not fix-Matrix [
          ...
          if Spatial-Mapping-Enabled [
            assign-Vector-element         ; calculates (predicted) vectors for pairwise associations that haven't been experienced yet
          ]
        ]
...
;        if j = num_inputs - 1 [set tracej reward_neg]
  ...
  ifelse i >= (num_inputs - 2) or j >= (num_inputs - 2) [
    ...
    matrix:set FAMatrix_timelags i j 2 * matrix:get FAMatrix_diff-A i j
    ...
    if Spatial-Mapping-Enabled and cross > 0.4 [
      let vsign 0
...
to assign-Reward-element
  ...
  ifelse matrix:get FAMatrix_timelags i j >= 0 [  ; if input2 follows/coincides with input1
    ...
    matrix:set FAMatrix_rewards i j (strength) * (forward-reward) ;0.5*
  ][
    let col_r matrix:get-column FAMatrix_rewards i
    let col_tl matrix:get-column FAMatrix_timelags i
    let col_length length col_r
    ...
    ;set backward-reward sum [reward] of combos with [input2 = my-input1 and timelag <= 0]
    ;set forward-reward sum [reward] of combos with [input1 = my-input2 and timelag <= 0]
    matrix:set FAMatrix_rewards i j (strength) * (backward-reward) ;0.5*
  ]
]
end
...
if i != j and i < (num_inputs - 3) and j < (num_inputs - 3)[
    ...
    while [k < (num_inputs - 3)] [
      ...
      ;if no association/imagined association between inputs j and k has been formed yet, then "imagine" it!
...
to calculate-path-integration
  set slitherometer slitherometer + speed
  if slitherometer > 0 and slitherometer mod stepsize = 0 [
    set vsum sum-vectors ( list vsum (list heading stepsize))
  ]
  if max traces >= 0.99 [
    set vsum [0 0]
    ...
    while [j < num_inputs] [
      if i != j and i < (num_inputs - 3) and j < (num_inputs - 3)[
        ...
        ifelse vdist = 0 [
          ...
          if order > 0 or experiment_mode = "Obstacle Avoidance Learning" [
            set left_dist item 1 subtract-vectors error-vector lsns_vec
            set right_dist item 1 subtract-vectors error-vector rsns_vec
            set imag_senses_left replace-item j imag_senses_left (item j imag_senses_left + 10 * strength * tracei * (1 / (left_dist + 1)))
...
to drag
  if mouse-down? [
    let nearest-items turtles with [breed != memory-pointers and distancexy mouse-xcor mouse-ycor < 3]
...
;-------------------------------------------------------------------------------------------------------
  to reset
    set Presentation-Mode false
    set Experiment_Mode "Spatial Sequence Learning"
    set immobilize false
