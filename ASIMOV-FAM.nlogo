  ...
  while [i < num_inputs] [
    while [j < num_inputs] [
      if i != j and i < (num_inputs - 3) and j < (num_inputs - 3)[
        ...
        if vdestination != [0 0] and abs(imageL_j + imageR_j)> 2.0 and abs(imageL_j - imageR_j) / abs(imageL_j + imageR_j) > 0.04 [ ;if there is significant deviation from memory vector, split memory vector into detour and righting vectors
          set detour-vector vsum
          set correction-vector subtract-vectors vdestination detour-vector
          set FAMatrix_vseq set-element FAMatrix_vseq i j (list detour-vector correction-vector)
        ]

        ;;    ******   CURRENT WORK IN PROGRESS, an alternative to the above code block, for multiple detour and correction vector sequence encoding ************
;        if vdestination != [0 0] and abs(imageL_j + imageR_j)> 2.0[
;          ifelse abs(imageL_j - imageR_j) > 0.10 [ ;if there is significant deviation from memory vector, split memory vector into detour and correction vectors
...
to predefine-memory-vector
  if ticks = 1 [
    ...
    matrix:set FAMatrix_vdist 0 2 58.008620049092706
    matrix:set FAMatrix_vdir 2 0 323.61564818416412
    matrix:set FAMatrix_vdist 2 0 58.008620049092706
...
For saving data and simulation states, NetLogo has its own "Export World" feature in NetLogo's topmost menu, and in addition to this we have provided functions for saving and loading environments (SAVE-ENV and LOAD-ENV on the left), and saving and loading agent variables (SAVE-AGENT-VARS and LOAD-AGENT-VARS below the middle environment panel).
